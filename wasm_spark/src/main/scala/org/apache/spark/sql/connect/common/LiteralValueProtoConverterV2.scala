...
object LiteralValueProtoConverterV2 {

  /**
   * Transforms literal value to the `proto.Literal.Builder`.
   *
   * @return
   *   proto.Literal.Builder
   */
  @scala.annotation.tailrec
  def toLiteralProtoBuilder(literal: Any): proto.Literal.Builder = {
    val builder = proto.Literal.newBuilder()

    def decimalBuilder(precision: Int, scale: Int, value: String) = {
      proto.Literal.Decimal.newBuilder().setPrecision(precision).setScale(scale).setValue(value)
    }

    def calendarIntervalBuilder(months: Int, days: Int, microseconds: Long) = {
      proto.Literal.CalendarInterval.newBuilder()
        .setMonths(months)
        .setDays(days)
    ...
    def arrayBuilder(array: Array[_]) = {
      val ab = proto.Literal.Array.newBuilder()
        .setElementType(DataTypeProtoConverterV2.toConnectProtoType(toDataType(array.getClass.getComponentType)))
    ...
    literal match {
      ...
      case null => builder.setNull(ProtoDataTypesV2.NullType)
  ...
  def toLiteralProtoBuilder(
    literal: Any,
    dataType: DataType): proto.Literal.Builder = {
    val builder = proto.Literal.newBuilder()

    def arrayBuilder(scalaValue: Any, elementType: DataType) = {
      val ab = proto.Literal.Array.newBuilder().setElementType(DataTypeProtoConverterV2.toConnectProtoType(elementType))
      scalaValue match {
        case a: Array[_] =>
          a.foreach(item => ab.addElements(LiteralValueProtoConverterV2.toLiteralProto(item, elementType)))
    ...
    def mapBuilder(scalaValue: Any, keyType: DataType, valueType: DataType) = {
      val mb = proto.Literal.Map.newBuilder()
        .setKeyType(DataTypeProtoConverterV2.toConnectProtoType(keyType))
        .setValueType(DataTypeProtoConverterV2.toConnectProtoType(valueType))
    ...
    def structBuilder(scalaValue: Any, structType: StructType) = {
      val sb = proto.Literal.Struct.newBuilder().setStructType(DataTypeProtoConverterV2.toConnectProtoType(structType))
    ...
    (literal, dataType) match {
      ...
      case (v: Option[_], x: DataType) =>
        if (v.isDefined) {
          toLiteralProtoBuilder(v.get)
        } else {
          val dt = DataTypeProtoConverterV2.toConnectProtoType(x)
          builder.setNull(dt)
          //builder.getNull.toBuilder.setNull(DataTypeProtoConverterV2.toConnectProtoType(dataType))
        }
      case _ => toLiteralProtoBuilder(literal)
    }
  }

  def create[T: TypeTag](v: T): proto.Literal.Builder = Try {
  ...
  /**
   * Transforms literal value to the `proto.Literal`.
   *
   * @return
   *   proto.Literal
   */
  def toLiteralProto(literal: Any): proto.Literal =
    toLiteralProtoBuilder(literal).build()

  def toLiteralProto(literal: Any, dataType: DataType): proto.Literal =
  ...
  def toCatalystValue(literal: proto.Literal): Any = {
    literal.getLiteralTypeCase match {
      case proto.Literal.LiteralTypeCase.NULL => null

      case proto.Literal.LiteralTypeCase.BINARY => literal.getBinary.toByteArray

      case proto.Literal.LiteralTypeCase.BOOLEAN => literal.getBoolean

      case proto.Literal.LiteralTypeCase.BYTE => literal.getByte.toByte

      case proto.Literal.LiteralTypeCase.SHORT => literal.getShort.toShort

      case proto.Literal.LiteralTypeCase.INTEGER => literal.getInteger

      case proto.Literal.LiteralTypeCase.LONG => literal.getLong

      case proto.Literal.LiteralTypeCase.FLOAT => literal.getFloat

      case proto.Literal.LiteralTypeCase.DOUBLE => literal.getDouble

      case proto.Literal.LiteralTypeCase.DECIMAL =>
        Decimal(literal.getDecimal.getValue)

      case proto.Literal.LiteralTypeCase.STRING => literal.getString

      case proto.Literal.LiteralTypeCase.DATE =>
        SparkDateTimeUtils.toJavaDate(literal.getDate)

      case proto.Literal.LiteralTypeCase.TIMESTAMP =>
        SparkDateTimeUtils.toJavaTimestamp(literal.getTimestamp)

      case proto.Literal.LiteralTypeCase.TIMESTAMP_NTZ =>
        SparkDateTimeUtils.microsToLocalDateTime(literal.getTimestampNtz)

      case proto.Literal.LiteralTypeCase.CALENDAR_INTERVAL =>
        new CalendarInterval(
          literal.getCalendarInterval.getMonths,
          literal.getCalendarInterval.getDays,
          literal.getCalendarInterval.getMicroseconds)

      case proto.Literal.LiteralTypeCase.YEAR_MONTH_INTERVAL =>
        SparkIntervalUtils.monthsToPeriod(literal.getYearMonthInterval)

      case proto.Literal.LiteralTypeCase.DAY_TIME_INTERVAL =>
        SparkIntervalUtils.microsToDuration(literal.getDayTimeInterval)

      case proto.Literal.LiteralTypeCase.ARRAY =>
  ...
  private def getConverter(dataType: proto.DataType): proto.Literal => Any = {
  ...
  def toCatalystArray(array: proto.Literal.Array): Array[_] = {
    def makeArrayData[T](converter: proto.Literal => T)(implicit
  ...
  def toCatalystMap(map: proto.Literal.Map): mutable.Map[_, _] = {
    def makeMapData[K, V](
      keyConverter: proto.Literal => K,
      valueConverter: proto.Literal => V)(implicit
                                                     tagK: ClassTag[K],
  ...
  def toCatalystStruct(struct: proto.Literal.Struct): Any = {
